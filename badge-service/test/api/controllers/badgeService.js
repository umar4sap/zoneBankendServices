...
var orgId = "test14nov";
var appPath = '/v2/components/badge-service/org/' + orgId;
...
describe('connection Service', function() {
    ...
    it('creating a badge', function(done) {
        request(server)
            .post(badgePath)
            .set('Accept', 'application/json')
            .set('Authorization', Bearer)
            .send(inputData.badgeCreateInput)
            ...
            .end(function(err, res) {
                ...
                badgeId = res.body.generated_keys[0];
                inputData.connectionCreateInput.badges[0].badgeId = badgeId
                done();
            });
    });
    it('updating a badge', function(done) {

        request(server)
            .put(badgePath + '/' + badgeId)
            .set('Accept', 'application/json')
            .set('Authorization', Bearer)
            .send(inputData.badgeUpdateInput)
            //.expect('Content-Type', 'application/json')
            .expect(200)
            .end(function(err, res) {
                should.not.exist(err);
    ...
    it('get all badge', function(done) {
        request(server)
            .get(badgePath)
            .set('Accept', 'application/json')
            .set('Authorization', Bearer)
    ...
    it('get one badge', function(done) {
        request(server)
            .get(badgePath + '/' + badgeId)
            .set('Accept', 'application/json')
            .set('Authorization', Bearer)
            //.expect('Content-Type', 'application/json')
            .expect(200)
    ...
    it('deleting a badge', function(done) {
        request(server)
            .delete(badgePath + '/' + badgeId)
            .set('Accept', 'application/json')
            .set('Authorization', Bearer)
            //.expect('Content-Type', 'application/json')
            .expect(200)
